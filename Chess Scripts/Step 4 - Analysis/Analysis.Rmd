...
ggplot(data = All_players, aes(x = Date, y = gain_loss)) + 
...
```{r, echo=FALSE}
codes <- fread("~/GitHub/FIDE/Chess Scripts/FIDE Country Codes/FIDE_codes.csv", 
               sep =  ",", header = TRUE)
...
countries <- c("Berundi", "Bhutan", "Burkina Faso", "Central African Republic", "Cambodia", 
               ...
               "Sierra Leonne", "Solomon Islands", "South Sudan", "Sao Tome and Principe", "Swaziland", 
               "East Timor (Timor-Leste)	", "Taiwan", "Trinidad and Tobago") 
...
Country_diff <- All_countries%>%
                ...
                filter(!is.na(Diff))%>%
                mutate(Country_name = c(codes$Country, Country)[match(Country, c(codes$Code, Country))],
                       Country_name = c(countries, Country_name)[match(Country_name, c(country_codes, Country_name))])
...
Country_diff%>%
filter(Country_name %in% Notable)%>%
ggplot() +
geom_line(aes(x = Date, y = Diff, colour = Country_name))+
theme_bw()
...
titled%>%
ggplot(aes(x=Date, y= n, group=Title, color=Title)) +
geom_line(size = 1.5) -> p9

titles_group%>%
ggplot(aes(x=Date, y= n, group=w_titles, color=w_titles)) +
geom_line(size = 1.5) -> p10

titles_group %>%
group_by(w_titles) %>%
mutate(Diff = n - lag(n))%>%
ggplot(aes(x=Date, y= Diff, group=w_titles, color=w_titles)) +
geom_line(size = 1.5) -> p11

Games%>%
ggplot() +
geom_line(aes(x=Date, y= roll), size = 1.5, color = "red")+
theme_bw() -> p12
...
titled <- FIDE%>%
          filter(Title %in% new, Date_numeric == max(Date_numeric))%>%
