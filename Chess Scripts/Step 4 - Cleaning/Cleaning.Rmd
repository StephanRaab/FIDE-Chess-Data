...
We also create a destination path with `dest` for later use so that we can export the files properly. `country_path` is a path to a dataset from which we will clean up the data on.

```{r}
path = "~/GitHub/FIDE/Chess Scripts/Step 2 - Reformat/Data csvs/"
country_path = "~/GitHub/FIDE/Chess Scripts/Step 3 - FIDE Country Codes/FIDE_codes.csv"
...
Below, we import and assign all of the datasets into memory. The only objects we need to hold onto is the data and several folder path references for later on.
