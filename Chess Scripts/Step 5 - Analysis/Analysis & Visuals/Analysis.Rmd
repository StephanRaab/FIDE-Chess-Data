...
ggplot(data = All_players, aes(x = Date_numeric, y = total_count)) + 
  ...
  theme(panel.grid.major = element_line(linetype = 'solid',colour = "white", size = 0.1), 
        panel.grid.minor = element_line(linetype = 'solid',colour = "white", size = 0.1))-> p1
...
ggplot(data = Active_player, aes(x = Date_numeric, y = total_count)) + 
  ...
  dark_theme_gray()+
  scale_y_continuous(labels = comma)+
  theme(panel.grid.major = element_line(linetype = 'solid',colour = "white", size = .1), 
        panel.grid.minor = element_line(linetype = 'solid',colour = "white", size = .1)) -> p2
...
ggplot(data = Inactive_player, aes(x = Date_numeric, y = total_count)) + 
  ...
  scale_y_continuous(labels = comma)+
  theme(panel.grid.major = element_line(linetype = 'solid',colour = "white", size = .1), 
        panel.grid.minor = element_line(linetype = 'solid',colour = "white", size = .1))-> p3
...
ggplot(data = All_players, aes(x = Date_numeric, y = gain_loss)) + 
  ...
  scale_y_continuous(labels = comma)+
  theme(panel.grid.major = element_line(linetype = 'solid',colour = "white", size = .1), 
        panel.grid.minor = element_line(linetype = 'solid',colour = "white", size = .1))-> p4
...
ggplot(data = Active_player, aes(x = Date_numeric, y = gain_loss)) + 
  ...
  scale_y_continuous(labels = comma)+
  theme(panel.grid.major = element_line(linetype = 'solid',colour = "white", size = .1), 
        panel.grid.minor = element_line(linetype = 'solid',colour = "white", size = .1))-> p5
...
ggplot(data = Inactive_player, aes(x = Date_numeric, y = gain_loss)) + 
  ...
  scale_y_continuous(labels = comma)+
  theme(panel.grid.major = element_line(linetype = 'solid',colour = "white", size = .1), 
        panel.grid.minor = element_line(linetype = 'solid',colour = "white", size = .1))-> p6
...
geom_hline(aes(yintercept = 0))-> p11
...
ggplot(aes(x= Country, y =n, color = Country))+
    ...
    facet_wrap(~Title)+
    dark_theme_gray()+
    theme(axis.text=element_text(size=6)) -> p16

strongest%>%
ggplot(aes(x= Country, y= average_rating, color = Country))+
    geom_bar(stat ="identity")+
    coord_flip()+
    xlab("Country")+
    ylab("Average Rating")+
    ...
    dark_theme_gray()+
    theme(axis.text=element_text(size=6)) -> p17
...
**Activity** is defined by if a given FIDE player had played a rated FIDE game within the past 12 months. If we take this into account, charts `C` and `E` show how the total active & inactive player count increase over time. Both charts show a fairly linear trend over time, but from 2007 to 2010 in each graph, there was a noticeable drop off in the active player base and increase in the inactive player base. I have my doubts on if there is faulty data here because chart `A` shows no irregularity during that time. 

This dip may be due to the economic crash during that time, but I need to do more exploration on this topic before making any definitive statements. (Exploring the dip by country is future work)

Charts `B`, `D` and `F` show the number of players gained and lost over time.

The most relevant of the 3 graphs is `B` which shows several instances where total player counts dropped off. I genuinely don't if the data is faulty because of my doing or if FIDE is providing incomplete data sets based off of the charts.
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, fig.height=10.5, fig.width= 9}
...
plot_grid(p12, p13, p14, p15, labels = c("K", "L", "M", "N")) 
```
...
- CMs and FMs vary greatly across rating categories because it has become much easier for lower rated players to acquire titles in youth tournaments and via interzonal tournaments.
...
#               na.omit()
# 
# years_increment = all_titled$Date_numeric%>%
#                   unique()%>%
#                  .[seq(1, length(.), 3)]
# 
# # find_hull <- function(df) df[chull(df$Age, df$Rating), ]
