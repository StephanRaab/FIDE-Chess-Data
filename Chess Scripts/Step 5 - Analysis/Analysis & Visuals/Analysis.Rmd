...
```{r, warning=FALSE, message=FALSE, echo=FALSE, fig.height= 6, cache=TRUE}
...
The most relevant of the 3 graphs is `B` which shows several instances where total player counts dropped off. I genuinely don't if the data is faulty because of my doing or if FIDE is providing incomplete datasets. 
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE, fig.height=10.5, fig.width= 9}
plot_grid(p7, p8, p9, p10, labels = c("G", "H", "I", "J"))
```

Each chart above shows how the Active and Inactive player's average rating and standard deviation have progressed over time. For the most part, it is a meaningless metric because FIDE has brought in more **lower** rated chess players into the player pool over time. 

This explains the slow decrease in average rating and increase in standard deviation over time.
...
```{r, echo = FALSE, warning=TRUE, message=TRUE, fig.height=7, fig.width=9, cache=TRUE}
p11
```
...
```{r, echo=FALSE, cache=TRUE, fig.height= 6.5, fig.width= 6}
p18
```
...
- There is a slight negative corrrelation, among all titled player groups, between Age and Rating. 
